...
class WatermarkApp:
    ...
    def setup_ui(self):
        ...
        control_frame = ttk.LabelFrame(main_frame, text="Watermark Settings", padding="10")
        control_frame.grid(row=1, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), padx=(0, 10))
        
        # Image upload section
        ttk.Label(control_frame, text="1. Upload Image:").grid(row=0, column=0, sticky=tk.W, pady=5)
        upload_btn = ttk.Button(control_frame, text="Browse Image", command=self.upload_image)
        upload_btn.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=5)
        
        # Watermark type selection
        ttk.Label(control_frame, text="2. Watermark Type:").grid(row=2, column=0, sticky=tk.W, pady=(20, 5))
        
        self.watermark_type = tk.StringVar(value="text")
        text_radio = ttk.Radiobutton(control_frame, text="Text", variable=self.watermark_type, 
                                    value="text", command=self.toggle_watermark_type)
        text_radio.grid(row=3, column=0, sticky=tk.W)
        
        logo_radio = ttk.Radiobutton(control_frame, text="Logo", variable=self.watermark_type, 
                                    value="logo", command=self.toggle_watermark_type)
        logo_radio.grid(row=4, column=0, sticky=tk.W)
        
        # Text watermark settings
        self.text_frame = ttk.LabelFrame(control_frame, text="Text Settings", padding="5")
        ...
        text_entry = ttk.Entry(self.text_frame, textvariable=self.watermark_text, width=20)
        text_entry.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=2)
        
        ttk.Label(self.text_frame, text="Font Size:").grid(row=2, column=0, sticky=tk.W, pady=(10, 0))
        size_scale = ttk.Scale(self.text_frame, from_=12, to=72, variable=self.watermark_size,
        ...
        ttk.Label(self.text_frame, text="Color:").grid(row=5, column=0, sticky=tk.W, pady=(10, 0))
        color_btn = ttk.Button(self.text_frame, text="Choose Color", command=self.choose_color)
        color_btn.grid(row=6, column=0, sticky=(tk.W, tk.E), pady=2)
        
        # Logo watermark settings
        self.logo_frame = ttk.LabelFrame(control_frame, text="Logo Settings", padding="5")
        self.logo_frame.grid(row=6, column=0, sticky=(tk.W, tk.E), pady=10)
        
        logo_btn = ttk.Button(self.logo_frame, text="Browse Logo", command=self.upload_logo)
        ...
        # Common settings
        ttk.Label(control_frame, text="3. Position:").grid(row=7, column=0, sticky=tk.W, pady=(20, 5))
        position_combo = ttk.Combobox(control_frame, textvariable=self.watermark_position,
        ...
        ttk.Label(control_frame, text="4. Opacity:").grid(row=9, column=0, sticky=tk.W, pady=(20, 5))
        opacity_scale = ttk.Scale(control_frame, from_=50, to=255, variable=self.watermark_opacity,
                                 orient=tk.HORIZONTAL, command=self.update_preview)
        ...
        # Buttons
        preview_btn = ttk.Button(control_frame, text="Preview Watermark", command=self.update_preview)
        preview_btn.grid(row=12, column=0, sticky=(tk.W, tk.E), pady=(20, 5))
        
        save_btn = ttk.Button(control_frame, text="Save Watermarked Image", command=self.save_image)
    ...
    def save_image(self):
        if not self.watermarked_image:
            messagebox.showwarning("Warning", "Please create a watermark preview first!")
            return
